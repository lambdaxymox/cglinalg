...
fn test_vector3_unit_vectors() {
    ...
    let (unit_unit_x, norm_unit_x) = Unit::new_with_magnitude(unit_x); 
    let (unit_unit_y, norm_unit_y) = Unit::new_with_magnitude(unit_y);
    let (unit_unit_z, norm_unit_z) = Unit::new_with_magnitude(unit_z);
...
fn test_vector_close_to_zero() {
    ...
    let expected = None;
    let result = Unit::try_new(vector, threshold);
