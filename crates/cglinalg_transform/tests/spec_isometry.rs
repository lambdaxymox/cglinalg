...
fn strategy_isometry2_from_range<S>(min_angle: S, max_angle: S, min_distance: S, max_distance: S) -> impl Strategy<Value = Isometry2<S>>
where
    S: SimdScalarFloat + Arbitrary,
{
    fn rescale<S>(value: S, min_value: S, max_value: S) -> S
    ...
    any::<(S, [S; 2])>().prop_map(move |(_angle, _vector)| {
        let translation = {
...
fn strategy_isometry3_from_range<S>(min_angle: S, max_angle: S, min_distance: S, max_distance: S) -> impl Strategy<Value = Isometry3<S>>
where
    S: SimdScalarFloat + Arbitrary,
{
    fn rescale<S>(value: S, min_value: S, max_value: S) -> S
    ...
    any::<(S, [S; 3], [S; 3])>().prop_map(move |(_angle, _axis, _vector)| {
        let translation = {
...
fn strategy_isometry2_f64_any() -> impl Strategy<Value = Isometry2<f64>> {
    let min_angle = 0_f64;
...
fn strategy_isometry3_f64_any() -> impl Strategy<Value = Isometry3<f64>> {
    let min_angle = 0_f64;
    let max_angle = 50_f64 * f64::two_pi();
    let min_distance = -1_000_000_f64;
    let max_distance = 1_000_000_f64;
...
/// norm(M * v) == norm(v)
/// ```
fn prop_approx_isometry_vector_preserves_norm<S, const N: usize>(
    m: Isometry<S, N>,
    v: Vector<S, N>,
    tolerance: S,
    max_relative: S,
...
/// ```
fn prop_approx_isometry_isometry_inverse<S, const N: usize, const NN: usize>(m: Isometry<S, N>, tolerance: S) -> Result<(), TestCaseError>
where
    S: SimdScalarFloat,
...
/// inverse(M) * (M * p) == p
/// ```
fn prop_approx_isometry_isometry_inverse_pointwise_point<S, const N: usize>(
    m: Isometry<S, N>,
    p: Point<S, N>,
    tolerance: S,
    max_relative: S,
...
/// inverse(M) * (M * v) == v
/// ```
fn prop_approx_isometry_isometry_inverse_pointwise_vector<S, const N: usize>(
    m: Isometry<S, N>,
    v: Vector<S, N>,
    tolerance: S,
    max_relative: S,
...
where
    S: SimdScalarFloat,
{
    let m1 = Isometry2::from_angle(angle1);
    let m2 = Isometry2::from_angle(angle2);
    let m3 = Isometry2::from_angle(angle1 + angle2);
    let lhs = m1 * m2;
    let rhs = m3;
...
/// ```
fn prop_approx_isometry3_composition_same_axis_equals_addition_of_angles<S>(
    axis: Unit<Vector3<S>>,
    angle1: Radians<S>,
    angle2: Radians<S>,
    tolerance: S,
...
where
    S: SimdScalarFloat,
{
    let m1 = Isometry3::from_axis_angle(&axis, angle1);
    let m2 = Isometry3::from_axis_angle(&axis, angle2);
    let m3 = Isometry3::from_axis_angle(&axis, angle1 + angle2);
    let lhs = m1 * m2;
    let rhs = m3;
...
mod isometry2_f64_invariant_props {
    ...
    proptest! {
        #[test]
        fn prop_approx_isometry_vector_preserves_norm(m in super::strategy_isometry2_f64_any(), v in super::strategy_vector_f64_any()) {
            let m: super::Isometry2<f64> = m;
            let v: super::Vector2<f64> = v;
            super::prop_approx_isometry_vector_preserves_norm(m, v, 1e-10, 1e-10)?
        }
    }
}

#[cfg(test)]
mod isometry2_f64_composition_props {
    use proptest::prelude::*;
    proptest! {
        #[test]
        fn prop_approx_isometry_isometry_inverse(m in super::strategy_isometry2_f64_any()) {
            let m: super::Isometry2<f64> = m;
            super::prop_approx_isometry_isometry_inverse(m, 1e-8)?
        }
        ...
        fn prop_approx_isometry_isometry_inverse_pointwise_point(
            ...
            super::prop_approx_isometry_isometry_inverse_pointwise_point(m, p, 1e-6, 1e-6)?
        }
        ...
        fn prop_approx_isometry_isometry_inverse_pointwise_vector(
            ...
            let v: super::Vector2<f64> = v;
            super::prop_approx_isometry_isometry_inverse_pointwise_vector(r, v, 1e-6, 1e-6)?
        }

        #[test]
        fn prop_approx_isometry2_composition_same_axis_equals_addition_of_angles(
            ...
            super::prop_approx_isometry2_composition_same_axis_equals_addition_of_angles(angle1, angle2, 1e-10)?
        }
    }
}

#[cfg(test)]
mod isometry3_f64_invariant_props {
    use proptest::prelude::*;
    proptest! {
        #[test]
        fn prop_approx_isometry_vector_preserves_norm(m in super::strategy_isometry3_f64_any(), v in super::strategy_vector_f64_any()) {
...
#[cfg(test)]
mod isometry3_f64_composition_props {
    use proptest::prelude::*;
    proptest! {
        #[test]
        fn prop_approx_isometry_isometry_inverse(m in super::strategy_isometry3_f64_any()) {
            let m: super::Isometry3<f64> = m;
            super::prop_approx_isometry_isometry_inverse(m, 1e-8)?
        }
        ...
        #[test]
        fn prop_approx_isometry3_composition_same_axis_equals_addition_of_angles(
            ...
            let angle1: super::Radians<f64> = angle1;
            let angle2: super::Radians<f64> = angle2;
            super::prop_approx_isometry3_composition_same_axis_equals_addition_of_angles(axis, angle1, angle2, 1e-10)?
        }
    }
}
